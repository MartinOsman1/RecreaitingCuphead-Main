...
public class PlayerJumpState : State
{
    ...
    public override void UpdateState()
    {
        if(!context.isDashing){
            context.rigidbody2D.linearVelocity = new Vector2(context.move * speed, context.rigidbody2D.linearVelocity.y);
        }
    ...
    void Parry(){
        ...
        if(hit.collider != null){
            ...
            context.rigidbody2D.linearVelocity = new Vector2(context.rigidbody2D.linearVelocity.x, context.jumpForce / 1.2f);
            canParry = true;
        }
    }
